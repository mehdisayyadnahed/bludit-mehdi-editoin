...
;(function(root){
    "use strict";
    var w = root, d = root.document, r = null;
            ...
            form.addEventListener("submit", function(event){
                ...
                ajax(SNICKER_PATH, "POST", data, function(json){
                    ...
                    if(list && data.status == "success" && "comment" in data){
                        if( r !== null ){
                            var item = d.getElementById("comment-" + r);
                            r = null;
                            item.insertAdjacentHTML("beforeend", data.comment);
                            item.scrollIntoView({ behavior: "smooth", block: "center" });
                        } else {
                            if (d.querySelector(".no-comments")){
                                d.querySelector(".no-comments").remove(self);
                            }
                            list.insertAdjacentHTML("beforeend", data.comment);

                            d.querySelector(".footer").scrollIntoView({ behavior: "smooth", block: "center" });
                        }
                        ...
                        showAlert("success", data.success, form.querySelector(".comment-header"));
                        if(captcha && !("captcha" in data)){
                            d.querySelector("input[name='comment[captcha]']").value = "";
                            data = "action=snicker&snicker=captcha&tokenCSRF=";
                        ...
                        showAlert("error", data.error, form.querySelector(".comment-header"));
        ...
        if(list){
            list.addEventListener("click", function(event){
                ...
                var comment = (function getComment(element){
                    ...
                    var reg = (String(element).match(/([a-zA-Z0-9]{32})/g));
                    if( reg !== null ){
                        r = reg;
                    }
                    list.querySelector(".comment").scrollIntoView({ behavior: "smooth", block: "center" });
                    return (parent.classList.contains("counter"))? parent: getComment(parent);
                })(event.target);
                ...
                var reply = d.createElement("DIV");
                    ...
                    reply.innerHTML = '<a href="' + window.location.href + '" class="reply-cancel"></a>'
                                    + '<div class="reply-title">' + 'پاسخ به نظر: ' + comment.querySelector(".author-username").innerText + '</div>'
